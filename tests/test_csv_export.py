...
def test_semicolon_csv_matches_german_display_and_keeps_one_line_per_record() -> None:
    ...
    rows = list(csv.reader(io.StringIO(csv_text, newline=""), delimiter=";", quotechar='"'))

    assert csv_bytes.startswith(b"\xef\xbb\xbf")
    assert csv_text.startswith('"Datum";"Zeitpunkt";"Mögliche Einflussfaktoren";')
    ...
    assert rows[1] == [
        ...
        "10:00 Uhr: Beginn. 16:00 Uhr: Höhepunkt.",
        "Nein",
    ]

...
apply_ui()
page_header(
    tr("Stärke und Dauer", "Intensity and duration"),
    tr("Wie stark die Kopfschmerzen waren, wie lange sie dauerten und ob beides zusammenhing.", "How intense the headaches were, how long they lasted, and whether the two were related."),
)
...
with database_session() as session:
    ...
    st.subheader(tr("Stärke und Dauer nach Monat", "Intensity and duration by month"))
    y_metric = st.segmented_control(
        tr("Was soll die Höhe der Punkte zeigen?", "What should the height of the points show?"),
        ["strength", "duration"],
        default="strength",
        required=True,
        format_func=lambda value: tr("Stärke", "Intensity") if value == "strength" else tr("Dauer", "Duration"),
    ...
    st.subheader(tr("Dauer der Kopfschmerzen nach Kalendertag", "Headache duration by calendar date"))
    ...
    selected_chart_period = st.date_input(
        tr("Zeitraum für dieses Diagramm", "Date range for this chart"),
    ...
    with left:
        st.subheader(tr("Wie häufig kam welche Stärke vor?", "How often did each intensity occur?"))
        st.plotly_chart(histogram(data.entries, "strength"), width="stretch", config=chart_config())
    with right:
        st.subheader(tr("Wie lange dauerten die Kopfschmerzen?", "How long did the headaches last?"))
        st.plotly_chart(histogram(data.entries, "duration"), width="stretch", config=chart_config())

    st.subheader(tr("Dauerten stärkere Kopfschmerzen länger?", "Did more intense headaches last longer?"))
    ...
    if correlation["n"] < 2 or coefficient is None:
        ...
        if strength < 0.2:
            interpretation = tr("Es ist kein klares gemeinsames Muster zu erkennen.", "There is no clear shared pattern.")
        elif coefficient > 0 and strength < 0.5:
            interpretation = tr("Stärkere Kopfschmerzen dauerten tendenziell etwas länger.", "More intense headaches tended to last somewhat longer.")
        elif coefficient > 0:
            interpretation = tr("Stärkere Kopfschmerzen dauerten meist länger.", "More intense headaches usually lasted longer.")
        elif strength < 0.5:
            interpretation = tr("Stärkere Kopfschmerzen dauerten tendenziell etwas kürzer.", "More intense headaches tended to be somewhat shorter.")
        else:
            interpretation = tr("Stärkere Kopfschmerzen dauerten meist kürzer.", "More intense headaches usually lasted less time.")
        correlation_text = tr(

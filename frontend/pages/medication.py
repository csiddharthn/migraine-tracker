...
apply_ui()
page_header(
    tr("Medikamente und Behandlung", "Medication and treatment"),
    tr(
...
with database_session() as session:
    ...
    overview[0].metric(tr("Tage mit Momeallerg", "Days with Momeallerg"), sum(record.momeallerg_nasal_spray for record in daily))
    overview[1].metric(tr("Tage mit Amitriptylin", "Days with amitriptyline"), sum(record.amitriptyline_neuraxpharm for record in daily))
    overview[2].metric(tr("Dokumentierte Aimovig-Injektionen", "Recorded Aimovig injections"), sum(record.aimovig_injection for record in daily))

    st.subheader(tr("Hat die Akutmedikation geholfen?", "Did the acute medication help?"))
    ...
    st.subheader(tr("Hinweis zur häufigen Einnahme von Akutmedikamenten", "Guidance on frequent use of acute medication"))
    ...
    triptan_day_word = tr("Tag" if triptan_days == 1 else "Tagen", "day" if triptan_days == 1 else "days")
    simple_day_word = tr("Tag" if simple_days == 1 else "Tagen", "day" if simple_days == 1 else "days")
    note = tr(
    ...
    st.caption(tr("Quelle: International Classification of Headache Disorders, ICHD-3, Abschnitt 8.2.", "Source: International Classification of Headache Disorders, ICHD-3, section 8.2."))
    st.link_button(
        tr("ICHD-3-Kriterien öffnen", "Open ICHD-3 criteria"),
        tr("https://ichd-3.org/de/8-kopfschmerz-zurueckzufuehren-auf-eine-substanz-oder-deren-entzug/8-2-kopfschmerz-zurueckzufuehren-auf-einen-medikamentenuebergebrauch/", "https://ichd-3.org/8-headache-attributed-to-a-substance-or-its-withdrawal/8-2-medication-overuse-headache-moh/"),

...
apply_ui()
page_header(tr("Einträge", "Entries"), tr("Neue Kopfschmerztage eintragen, vorhandene Angaben ändern und mögliche Auslöser verwalten.", "Record new headache days, change existing information, and manage possible triggers."))
...
with database_session() as session:
    ...
    new_tab, ai_tab, edit_tab, trigger_tab = st.tabs(
        [
            ...
            tr("KI-gestützter Eintrag", "AI-assisted entry"),
            tr("Eintrag bearbeiten", "Edit entry"),
            tr("Auslöser verwalten", "Manage triggers"),
        ]
    )
    ...
    with new_tab:
        ...
        if payload is not None:
            try:
                ...
                session.commit()
                clear_data_cache()
                st.success(tr(f"Der Eintrag für den {format_date(entry.entry_date)} wurde gespeichert.", f"The entry for {format_date(entry.entry_date)} was saved."))
    ...
    with edit_tab:
        ...
        if not entries:
            st.info(tr("Noch keine Einträge vorhanden.", "No entries exist yet."))
        else:
            search_col, date_col = st.columns([2, 1])
            search = search_col.text_input(tr("Suchen", "Search"), placeholder=tr("Notiz, Medikation oder Auslöser", "Note, medication, or trigger"))
            selected_date = date_col.date_input(tr("Nur dieses Datum", "Only this date"), value=None, format=date_input_format())
            filtered = []
            ...
            for entry in entries:
                haystack = " ".join(
                    [
                        ...
                        " ".join(localized_trigger_label(trigger.trigger_code, trigger.definition.label) for trigger in entry.triggers),
                    ]
                ).lower()
            ...
            if not filtered:
                st.warning(tr("Keine passenden Einträge gefunden.", "No matching entries found."))
            else:
                entries_by_id = {entry.id: entry for entry in filtered}
                selected_id = st.selectbox(
                    tr("Eintrag auswählen", "Select entry"),
                    list(entries_by_id),
                    format_func=lambda entry_id: tr(
                ...
                if entry is not None:
                    st.caption(f"{tr('Zuletzt aktualisiert', 'Last updated')}: {format_datetime_value(entry.updated_at)}")
                    ...
                    if payload is not None:
                        try:
                            ...
                            session.commit()
                            clear_data_cache()
                            st.success(tr(f"Der Eintrag für den {format_date(updated.entry_date)} wurde aktualisiert.", f"The entry for {format_date(updated.entry_date)} was updated."))
                        except (DuplicateEntryError, ValueError) as exc:
                            session.rollback()
                            st.error(error_message(exc))

                    with st.expander(tr("Automatisch aus der Notiz erkannte Angaben prüfen", "Review information recognised automatically from the note"), expanded=False):
                        ...
                        if interpretation_changes is not None:
                            ...
                            session.commit()
                            clear_data_cache()
                            st.success(tr("Die geprüften Angaben aus der Notiz wurden gespeichert.", "The reviewed information from the note was saved."))
    ...
    with trigger_tab:
        ...
        st.subheader(tr("Neuen Auslöser anlegen", "Add a new trigger"))
        st.caption(tr("Der Auslöser wird dem gemeinsamen Katalog hinzugefügt und steht allen Personenprofilen zur Verfügung.", "The trigger is added to the shared catalogue and is available to all people profiles."))
        with st.form("create_trigger"):
            trigger_label = st.text_input(tr("Bezeichnung", "Label"), placeholder=tr("Zum Beispiel: Unregelmäßige Mahlzeit", "For example: Irregular meal"))
            new_trigger_description = st.text_area(tr("Beschreibung (optional)", "Description (optional)"), height=100)
            create_trigger = st.form_submit_button(tr("Auslöser anlegen", "Add trigger"), type="primary", icon=":material/add:")
        if create_trigger:
            try:
                trigger = trigger_service.create(trigger_label, new_trigger_description)
                session.commit()
                clear_data_cache()
                st.session_state["created_trigger_message"] = tr(f"Der Auslöser „{trigger.label}“ wurde angelegt.", f"The trigger “{trigger.label}” was added.")
                st.rerun()
            except (DuplicateTriggerError, ValueError) as exc:
                session.rollback()
                st.error(error_message(exc))

        st.subheader(tr("Vorhandene Auslöser", "Existing triggers"))
        trigger_rows = [
            {
                tr("Bezeichnung", "Label"): localized_trigger_label(trigger.code, trigger.label),
                tr("Beschreibung", "Description"): trigger_description(trigger.code, trigger.description),
                tr("Status", "Status"): tr("Aktiv", "Active") if trigger.active else tr("Inaktiv", "Inactive"),
            }

...
def _clock_text(minutes: int, *, lang: str) -> str:
    ...
    if day_offset == 0:
        return tr(cfg, f"{clock} Uhr", clock, lang=lang)
    return tr(cfg, f"{clock} Uhr ({day_offset} Tag später)", f"{clock} ({day_offset} day later)", lang=lang) if day_offset == 1 else tr(cfg, f"{clock} Uhr ({day_offset} Tage später)", f"{clock} ({day_offset} days later)", lang=lang)
...
def _clock_axis_text(hours: int, *, lang: str) -> str:
    ...
    if day_offset == 0:
        return clock
    return tr(cfg, f"+{day_offset} T<br>{clock}", f"+{day_offset} d<br>{clock}", lang=lang)
...
def observation_days_bar(
    ...
    *,
    ...
    lang = lang or current_language(cfg)
    total_days = headache_days + headache_free_days
    if total_days <= 0:
        return _empty_figure(tr(cfg, "Keine Beobachtungstage vorhanden.", "No observation days are available.", lang=lang), height=210)

    segments = [
        (tr(cfg, "Kopfschmerztage", "Headache days", lang=lang), headache_days, COLORS["amber"]),
        (tr(cfg, "Kopfschmerzfreie Tage", "Headache-free days", lang=lang), headache_free_days, COLORS["teal"]),
    ]
    fig = go.Figure()
    for label, days, color in segments:
        ...
        fig.add_trace(
            go.Bar(
                x=[days],
                y=[tr(cfg, "Beobachtungszeitraum", "Observation period", lang=lang)],
                name=label,
                orientation="h",
                marker_color=color,
                ...
                customdata=[[format_number(cfg, share, lang=lang)]],
                hovertemplate=(
                    f"<b>{label}</b><br>"
                    + tr(cfg, "Tage", "Days", lang=lang)
                    + ": %{x}<br>"
                    + tr(cfg, "Anteil am gesamten Zeitraum", "Share of the full period", lang=lang)
...
def monthly_metric_scatter(
    ...
    *,
    ...
    if not entries:
        return _empty_figure(tr(cfg, "Im gewählten Zeitraum wurden keine Kopfschmerztage eingetragen.", "No headache days were entered in the selected period.", lang=lang), height=360)
                ...
                points.append(
                    {
                        ...
                        "date": format_date_value(cfg, entry.entry_date, lang=lang),
                    }
                )
    ...
    if y_metric == "strength":
        ...
        colorbar = dict(title=tr(cfg, "Dauer in Stunden", "Duration in hours", lang=lang), thickness=18)
        y_title = tr(cfg, "Stärke von 1 bis 10", "Intensity from 1 to 10", lang=lang)
    else:
        ...
        y_title = tr(cfg, "Dauer in Stunden", "Duration in hours", lang=lang)
    ...
    fig = go.Figure(
        go.Scatter(
            ...
            showlegend=False,
            customdata=[
                [point["date"], point["strength"], format_number(cfg, point["duration"], lang=lang)]
                for point in points
            ],
            hovertemplate=(
                "%{customdata[0]}<br>"
                + tr(cfg, "Stärke", "Intensity", lang=lang)
                + ": %{customdata[1]} "
                + tr(cfg, "von 10", "out of 10", lang=lang)
                + "<br>"
                + tr(cfg, "Dauer", "Duration", lang=lang)
                + ": %{customdata[2]} "
                + tr(cfg, "Stunden", "hours", lang=lang)
                + "<extra></extra>"
            ),
        )
    )
    ...
    fig.update_xaxes(
        ...
        ticktext=[month_label(cfg, month, lang=lang) for month in month_keys],
        range=[-0.45, len(month_keys) - 0.55],
        title=tr(cfg, "Monat", "Month", lang=lang),
    )
    if y_metric == "strength":
...
def _strength_colorbar(lang: str) -> dict:
    return {
        "title": tr(cfg, "Stärke von 1 bis 10", "Intensity from 1 to 10", lang=lang),
        "tickmode": "array",
...
def monthly_frequency_chart(monthly: list[dict], *, lang: str | None = None) -> go.Figure:
    lang = lang or current_language(cfg)
    if not monthly:
        return _empty_figure(tr(cfg, "Für diesen Zeitraum liegen keine Monatswerte vor.", "No monthly values are available for this period.", lang=lang))

    labels = [month_label(cfg, item["key"], lang=lang) for item in monthly]
    common = [
        [
            item["observed_days"],
            format_number(cfg, item["rate_per_30"], lang=lang),
            format_number(cfg, item["avg_strength"], lang=lang),
        ]
        for item in monthly
    ]
    fig = go.Figure(
        go.Bar(
            ...
            name=tr(cfg, "Kopfschmerztage", "Headache days", lang=lang),
            ...
            hovertemplate=(
                "<b>%{x}</b><br>"
                + tr(cfg, "Kopfschmerztage", "Headache days", lang=lang)
                + ": %{y}<br>"
                + tr(cfg, "Kalendertage in diesem Monat", "Calendar days in this month", lang=lang)
                + ": %{customdata[0]}<br>"
                + tr(cfg, "Umgerechnet auf 30 Tage", "Converted to a 30-day period", lang=lang)
                + ": %{customdata[1]}<br>"
                + tr(cfg, "Durchschnittliche Stärke", "Average intensity", lang=lang)
                + ": %{customdata[2]} "
                + tr(cfg, "von 10", "out of 10", lang=lang)
                + "<extra></extra>"
            ),
    ...
    fig.update_yaxes(title_text=tr(cfg, "Kopfschmerztage", "Headache days", lang=lang), dtick=1, rangemode="tozero")
    fig.update_xaxes(title_text=tr(cfg, "Monat", "Month", lang=lang), type="category", categoryorder="array", categoryarray=labels)
    return _style(fig, height=360)
...
def rolling_line(points: list[dict], value: str, label: str, color: str = "teal", *, lang: str | None = None) -> go.Figure:
    lang = lang or current_language(cfg)
    if not points:
        return _empty_figure(tr(cfg, "Für diesen Zeitraum liegen noch keine Verlaufswerte vor.", "No trend values are available for this period yet.", lang=lang), height=320)
    frame = pd.DataFrame(points)
    fig = px.line(frame, x="date", y=value, markers=False, labels={"date": tr(cfg, "Datum", "Date", lang=lang), value: label}, color_discrete_sequence=[COLORS[color]])
...
def strength_duration_scatter(entries: Iterable[MigraineEntry], *, lang: str | None = None) -> go.Figure:
    ...
    if not entries:
        return _empty_figure(tr(cfg, "Im gewählten Zeitraum wurden keine Kopfschmerztage eingetragen.", "No headache days were entered in the selected period.", lang=lang), height=420)
    frame = pd.DataFrame(
        {
            "strength": [entry.strength for entry in entries],
            "duration": [float(entry.duration_hours) for entry in entries],
            "date": [format_date_value(cfg, entry.entry_date, lang=lang) for entry in entries],
        }
    )
    fig = px.scatter(
        frame,
        ...
        labels={
            "strength": tr(cfg, "Stärke von 1 bis 10", "Intensity from 1 to 10", lang=lang),
            "duration": tr(cfg, "Dauer in Stunden", "Duration in hours", lang=lang),
        },
    )
    fig.update_traces(
        marker_size=11,
        ...
        if slope is not None and intercept is not None:
            fig.add_trace(
                go.Scatter(
                    ...
                    name=tr(cfg, "Berechnete Trendlinie", "Calculated trend line", lang=lang),
                    showlegend=False,
                    hoverinfo="skip",
                )
            )
...
def duration_by_date_bar(
    ...
    *,
    ...
    if not items and (start_date is None or end_date is None):
        return _empty_figure(tr(cfg, "Im gewählten Zeitraum wurden keine Kopfschmerztage eingetragen.", "No headache days were entered in the selected period.", lang=lang), height=390)
    ...
    for calendar_date in calendar_dates:
        ...
        if entry is None:
            ...
            strengths.append(1)
            statuses.append(tr(cfg, "Kein Kopfschmerz eingetragen", "No headache recorded", lang=lang))
            continue
        duration = float(entry.duration_hours)
        ...
        statuses.append(
            tr(cfg, 
                f"Dauer: {format_number(cfg, duration, lang=lang)} Stunden · Stärke: {entry.strength} von 10",
                f"Duration: {format_number(cfg, duration, lang=lang)} hours · Intensity: {entry.strength} out of 10",
                lang=lang,
            )
        )
    frame = pd.DataFrame(
        {
            ...
            "date_text": [format_date_value(cfg, calendar_date, lang=lang) for calendar_date in calendar_dates],
            "duration": durations,
            "strength": strengths,
            "status": statuses,
        }
    ...
    fig.update_xaxes(
        title=tr(cfg, "Kalendertag", "Calendar date", lang=lang),
        type="category",
        categoryorder="array",
    ...
    fig.update_yaxes(title=tr(cfg, "Dauer in Stunden", "Duration in hours", lang=lang), rangemode="tozero")
    fig.update_coloraxes(colorbar=dict(title=tr(cfg, "Stärke von 1 bis 10", "Intensity from 1 to 10", lang=lang), tickvals=list(range(1, 11))))
...
def histogram(entries: Iterable[MigraineEntry], field: str, *, lang: str | None = None) -> go.Figure:
    ...
    items = list(entries)
    if not items:
        return _empty_figure(tr(cfg, "Im gewählten Zeitraum wurden keine Kopfschmerztage eingetragen.", "No headache days were entered in the selected period.", lang=lang))
    if field == "strength":
        frame = pd.DataFrame({"strength": [item.strength for item in items]})
        fig = px.histogram(frame, x="strength", nbins=10, labels={"strength": tr(cfg, "Stärke von 1 bis 10", "Intensity from 1 to 10", lang=lang)}, color_discrete_sequence=[COLORS["blue"]])
        fig.update_xaxes(dtick=1, range=[0.5, 10.5])
        fig.update_yaxes(title=tr(cfg, "Anzahl der Kopfschmerztage", "Number of headache days", lang=lang))
    else:
        frame = pd.DataFrame({"duration": [float(item.duration_hours) for item in items]})
        fig = px.histogram(frame, x="duration", nbins=min(12, max(4, len(items) // 2)), labels={"duration": tr(cfg, "Dauer in Stunden", "Duration in hours", lang=lang)}, color_discrete_sequence=[COLORS["blue"]])
        fig.update_yaxes(title=tr(cfg, "Anzahl der Kopfschmerztage", "Number of headache days", lang=lang))
...
def horizontal_bar(items: list[dict], *, value: str = "count", label: str = "label", color: str = "teal", height: int | None = None, lang: str | None = None) -> go.Figure:
    ...
    fig = go.Figure(
        go.Bar(
            ...
            hovertemplate=f"<b>%{{customdata[0]}}</b><br>{tr(cfg, 'Kopfschmerztage', 'Headache days', lang=lang)}: %{{x}}<extra></extra>",
        )
    )
    fig.update_xaxes(nticks=6, tickformat="d", rangemode="tozero", title=tr(cfg, "Anzahl der Kopfschmerztage", "Number of headache days", lang=lang))
...
def pattern_source_chart(trigger_items: list[dict], context_items: list[dict], *, lang: str | None = None) -> go.Figure:
    ...
    if not trigger_items and not context_items:
        return _empty_figure(tr(cfg, "Im gewählten Zeitraum wurden keine möglichen Auslöser oder Begleitumstände eingetragen.", "No possible triggers or accompanying circumstances were entered in the selected period.", lang=lang), height=360)
    ...
    fig = go.Figure()
    sources = (
        (trigger_items, tr(cfg, "Im Formular ausgewählt", "Selected in the form", lang=lang), COLORS["teal"]),
        (context_items, tr(cfg, "In den Notizen erwähnt", "Mentioned in the notes", lang=lang), COLORS["blue"]),
    )
    for items, source_label, color in sources:
        ...
        customdata = [
            [
                ...
                f"{item['share']:.0%}",
                format_number(cfg, item["avg_strength"], lang=lang),
                format_number(cfg, item["avg_duration"], lang=lang),
            ]
            for item in ordered
        ]
        fig.add_trace(
            go.Bar(
                ...
                hovertemplate=(
                    ...
                    + tr(cfg, "Kopfschmerztage", "Headache days", lang=lang)
                    + ": %{x}<br>"
                    + tr(cfg, "Anteil aller Kopfschmerztage", "Share of all headache days", lang=lang)
                    + ": %{customdata[1]}<br>"
                    + tr(cfg, "Durchschnittliche Stärke", "Average intensity", lang=lang)
                    + ": %{customdata[2]} "
                    + tr(cfg, "von 10", "out of 10", lang=lang)
                    + "<br>"
                    + tr(cfg, "Durchschnittliche Dauer", "Average duration", lang=lang)
                    + ": %{customdata[3]} "
                    + tr(cfg, "Stunden", "hours", lang=lang)
                    + "<extra></extra>"
                ),
...
def weekday_rate_chart(items: list[dict], *, lang: str | None = None) -> go.Figure:
    lang = lang or current_language(cfg)
    if not items:
        return _empty_figure(tr(cfg, "Keine Wochentagswerte vorhanden.", "No weekday values are available.", lang=lang))
    ...
    fig = go.Figure(
        go.Bar(
            ...
            hovertemplate=(
                "<b>%{x}</b><br>"
                + tr(cfg, "Kopfschmerzen an", "Headache on", lang=lang)
                + ": %{customdata[0]} "
                + tr(cfg, "von", "of", lang=lang)
                + " %{customdata[1]} "
                + tr(cfg, "Tagen", "days", lang=lang)
                + "<br>"
                + tr(cfg, "Anteil", "Share", lang=lang)
                + ": %{y:.0f}%<extra></extra>"
            ),
        )
    )
    ...
    fig.update_xaxes(
        title=tr(cfg, "Wochentag", "Weekday", lang=lang),
        tickmode="array",
        tickvals=weekday_labels,
        ticktext=weekday_labels,
        tickangle=-25,
    )
    fig.update_yaxes(title=tr(cfg, "Anteil mit Kopfschmerzen", "Percentage with headache", lang=lang), ticksuffix="%", range=[0, min(100, max(10, maximum * 1.28))])
...
def medication_effectiveness_chart(items: list[dict], *, lang: str | None = None) -> go.Figure:
    lang = lang or current_language(cfg)
    if not items:
        return _empty_figure(tr(cfg, "Keine Akutmedikation dokumentiert.", "No acute medication is documented.", lang=lang))
    ...
    responses = (
        ("helped_yes", tr(cfg, "Hat geholfen", "Helped", lang=lang), COLORS["teal"]),
        ("helped_partial", tr(cfg, "Hat teilweise geholfen", "Helped partly", lang=lang), COLORS["amber"]),
        ("helped_no", tr(cfg, "Hat nicht geholfen", "Did not help", lang=lang), COLORS["red"]),
        ("undocumented", tr(cfg, "Wirkung nicht eingetragen", "Effect not entered", lang=lang), "#a5adb8"),
    )
    fig = go.Figure()
    for key, response_label, color in responses:
        ...
        fig.add_trace(
            go.Bar(
                ...
                textposition="inside",
                customdata=[[item["label"], item["days"], format_number(cfg, item["avg_strength"], lang=lang)] for item in ordered],
                hovertemplate=(
                    "<b>%{customdata[0]}</b><br>"
                    + tr(cfg, "Tage mit dieser Bewertung", "Days with this assessment", lang=lang)
                    + ": %{x}<br>"
                    + tr(cfg, "Tage mit dieser Medikation", "Days with this medication", lang=lang)
                    + ": %{customdata[1]}<br>"
                    + tr(cfg, "Durchschnittliche Stärke an diesen Tagen", "Average intensity on these days", lang=lang)
                    + ": %{customdata[2]} "
                    + tr(cfg, "von 10", "out of 10", lang=lang)
                    + "<extra></extra>"
                ),
    ...
    fig.update_xaxes(dtick=1, tickformat="d", title=tr(cfg, "Tage mit Akutmedikation", "Days with acute medication", lang=lang), rangemode="tozero")
...
def completeness_chart(items: list[dict], *, lang: str | None = None) -> go.Figure:
    lang = lang or current_language(cfg)
    if not items:
        return _empty_figure(tr(cfg, "Keine Vollständigkeitswerte vorhanden.", "No completeness values are available.", lang=lang))
    ...
    fig = go.Figure(
        go.Bar(
            ...
            hovertemplate=(
                "<b>%{customdata[0]}</b><br>"
                + tr(cfg, "Ausgefüllt bei", "Completed for", lang=lang)
                + ": %{customdata[1]} "
                + tr(cfg, "von", "of", lang=lang)
                + " %{customdata[2]} "
                + tr(cfg, "relevanten Einträgen", "relevant entries", lang=lang)
                + "<br>"
                + tr(cfg, "Anteil", "Share", lang=lang)
                + ": %{x:.0f}%<extra></extra>"
            ),
    ...
    fig.update_xaxes(title=tr(cfg, "Anteil der ausgefüllten Angaben", "Share of completed information", lang=lang), ticksuffix="%", range=[0, 100])
    fig.update_yaxes(title="")
...
def attack_timeline(data: AnalyticsDataset, month_key: str, *, lang: str | None = None) -> go.Figure:
    ...
    while current <= last:
        days.append(current)
        current += timedelta(days=1)
    labels = [format_date_value(cfg, day, lang=lang) for day in sorted(days, reverse=True)]
    fig = go.Figure()
    fig.add_trace(
        go.Scatter(
            x=[0] * len(labels),
    ...
    for day in sorted(days, reverse=True):
        ...
        if onset_minute is not None and end_minute is not None:
            ...
            if end_was_calculated:
                end_text += tr(cfg, " (aus der eingetragenen Dauer berechnet)", " (calculated from recorded duration)", lang=lang)
            fig.add_trace(
                go.Bar(
                    x=[duration],
                    y=[format_date_value(cfg, day, lang=lang)],
                    base=[start],
                    width=[0.72],
                    orientation="h",
                    marker=dict(
                    ...
                    customdata=[
                        [
                            entry.strength,
                            format_number(cfg, float(entry.duration_hours), lang=lang),
                    ...
                    hovertemplate=(
                        ...
                        + tr(cfg, "von 10", "out of 10", lang=lang)
                        + "<br>"
                        + tr(cfg, "Eingetragene Dauer", "Recorded duration", lang=lang)
                        + ": %{customdata[1]} "
                        + tr(cfg, "Stunden", "hours", lang=lang)
                        + "<br>"
                        + tr(cfg, "Beginn", "Onset", lang=lang)
                        + ": %{customdata[2]}<br>"
                        + tr(cfg, "Ende", "End", lang=lang)
        ...
        if peak is not None:
            fig.add_trace(
                go.Scatter(
                    ...
                    name=tr(cfg, "Höhepunkt", "Peak", lang=lang),
                    legendgroup="peak",
                    showlegend=not peak_legend_added,
                    customdata=[[_clock_text(peak, lang=lang)]],
                    hovertemplate=tr(cfg, "Höhepunkt", "Peak", lang=lang) + ": %{customdata[0]}<extra></extra>",
                )
            )
    ...
    fig.update_xaxes(
        ...
        tickfont=dict(size=10),
        automargin=True,
        title=tr(cfg, "Uhrzeit", "Time", lang=lang),
    )

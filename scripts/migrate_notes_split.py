...
def migrate():
    ...
    for row in rows:
        ...
        timeline_text = ""
        # Rebuild timeline text from parsed timeline rows
        from backend.note_interpretation.structured_notes import format_structured_notes
        timeline_text = format_structured_notes(parsed) if parsed.timeline else (notes_text or "")
        # For simplicity: put full original notes into timeline_notes, split rest manually
        # Better approach: use parsed sections
        timeline_notes = notes_text or ""
